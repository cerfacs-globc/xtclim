...
def xr_to_ndarray(xr_dset: xr.Dataset, sq_coords: dict) -> (np.ndarray, np.array, str):
    """
...
def sftlf_to_ndarray(
    xr_dset: xr.Dataset, sq_coords: dict
...
def get_extrema(histo_dataset: np.ndarray, proj_dataset: np.ndarray) -> np.array:
    """
...
def normalize(nd_dset: np.ndarray, extrema: np.array) -> np.ndarray:
    """
...
def split_train_test(
    nd_dset: np.ndarray, time_list: np.array, train_proportion: float = 0.8
...
def split_date(
    nd_dset: np.ndarray, time_list: np.array, year: int = 1950
) -> (np.ndarray, np.ndarray, np.array, np.array):
    """
    Splits a data set into train and test data (and time),
...
def ndarray_to_2d(temp_dset: np.ndarray, land_prop: np.ndarray) -> np.ndarray:
    """
...
train_temp, test_temp, train_time, test_time = split_train_test(
    temp_histo_norm, time_list
)

# Add up split data and land-sea mask
total_train = ndarray_to_2d(train_temp, land_prop)
total_test = ndarray_to_2d(test_temp, land_prop)
total_proj = ndarray_to_2d(temp_proj_norm, land_prop)
