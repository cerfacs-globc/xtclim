...
if past_evaluation:
    for season in ["winter_", "spring_", "summer_", "autumn_"]:
        ...
        cvae_model.load_state_dict(
            torch.load(f"../outputs/cvae_model_{season}1d_{n_memb}memb.pth")
        )
        ...
        pd.DataFrame(tot_train_losses).to_csv(
            f"../outputs/train_losses_{season}1d_{n_memb}memb.csv"
        )
        pd.DataFrame(tot_test_losses).to_csv(
            f"../outputs/test_losses_{season}1d_{n_memb}memb.csv"
        )
...
if future_evaluation:
    for season in ["winter_", "spring_", "summer_", "autumn_"]:
        ...
        for scenario in ["370"]:
            ...
            proj_time = pd.read_csv(
                f"../input/dates_proj_{season}data_{n_memb}memb.csv"
            )
            proj_data = np.load(
                f"../input/preprocessed_1d_proj{scenario}_{season}data_{n_memb}memb.npy"
            )
            n_proj = len(proj_data)
            projset = [
                (
                    torch.from_numpy(np.reshape(proj_data[i], (2, 32, 32))),
                    proj_time["0"][i],
                )
            ...
            pd.DataFrame(tot_proj_losses).to_csv(
                f"../outputs/proj{scenario}_losses_{season}1d_{n_memb}memb.csv"
            )
            print(
