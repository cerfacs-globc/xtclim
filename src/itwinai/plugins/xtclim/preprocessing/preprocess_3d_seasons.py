...
def preprocess_3d_seasons(config_path = "./xtclim.json"):
    ...
    train_images = np.load(input_path+"/preprocessed_2d_train_data_allssp.npy")
    test_images = np.load(input_path+"/preprocessed_2d_test_data_allssp.npy")
    train_time = pd.read_csv(input_path+"/dates_train_data.csv")
    test_time = pd.read_csv(input_path+"/dates_test_data.csv")
    ...
    def season_split(
        ...
        np.save(
            input_path+f"/preprocessed_1d_{dataset_type}{scenario}_winter_data_{n_memb}memb.npy",
            winter_images,
        )
        np.save(
            input_path+f"/preprocessed_1d_{dataset_type}{scenario}_spring_data_{n_memb}memb.npy",
            spring_images,
        )
        np.save(
            input_path+f"/preprocessed_1d_{dataset_type}{scenario}_summer_data_{n_memb}memb.npy",
            summer_images,
        )
        np.save(
            input_path+f"/preprocessed_1d_{dataset_type}{scenario}_autumn_data_{n_memb}memb.npy",
            autumn_images,
        )
        pd.DataFrame(winter_time).to_csv(
            input_path+f"/dates_{dataset_type}_winter_data_{n_memb}memb.csv"
        )
        pd.DataFrame(spring_time).to_csv(
            input_path+f"/dates_{dataset_type}_spring_data_{n_memb}memb.csv"
        )
        pd.DataFrame(summer_time).to_csv(
            input_path+f"/dates_{dataset_type}_summer_data_{n_memb}memb.csv"
        )
        pd.DataFrame(autumn_time).to_csv(
            input_path+f"/dates_{dataset_type}_autumn_data_{n_memb}memb.csv"
        )
    ...
    test_season_images, test_season_time = season_split(test_images, test_time, "test", n_memb, input_path)
